...
## Recursive and Self-Referencing Blocks

Payload supports recursive and self-referencing blocks, allowing you to create complex nested content structures. This includes:

- **Self-referencing blocks**: A block that can contain instances of itself
- **Mutually recursive blocks**: Multiple blocks that reference each other in a cycle

### Self-Referencing Block Example
...
const MenuBlock: Block = {
  ...
  fields: [
    ...
    },
    {
      name: 'submenu',
      type: 'blocks',
      blockReferences: ['menu'], // Self-reference to create nested menus
    },
  ],
}
```

### Mutually Recursive Blocks Example
...
const SectionBlock: Block = {
  ...
  fields: [
    ...
    },
    {
      name: 'columns',
      type: 'blocks',
      blockReferences: ['column'], // References ColumnBlock
    },
  ],
}
...
const ColumnBlock: Block = {
  ...
  fields: [
    ...
    },
    {
      name: 'content',
      type: 'blocks', 
      blockReferences: ['section'], // References SectionBlock (creates cycle)
    },
  ],
}
```
...
When using recursive blocks, Payload automatically generates recursive TypeScript interfaces:

```ts
// Generated types for self-referencing block
interface MenuBlock {
  blockType: 'menu'
  title: string
  submenu?: MenuBlock[] // Recursive reference
}

// Generated types for mutually recursive blocks
...
interface ColumnBlock {
  blockType: 'column'
  width?: number
  content?: SectionBlock[] // Mutual reference
}
```

<Banner type="success">
  **Note:** Payload's type generation system automatically detects cycles and prevents infinite recursion, ensuring your TypeScript types compile correctly and your admin interface loads without issues.
</Banner>
